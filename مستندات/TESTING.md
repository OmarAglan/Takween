...
- DAG بدورة -> كود `1` وتشخيص المسار الكامل `الوحدة_١ -> الوحدة_٢ -> الوحدة_٣ -> الوحدة_١`.
...
- بيان افتراضي لنظم -> البصمة تطابق `nazm-api-v1` وتدخل إيصال الهوية ودليل
  المفتاح؛ إعادة البناء تعيد استخدام الكائنات بنفس البصمة.
...
- هدف مختلط `.baa`/`.نظم` بالافتراضي أو `المجمع = "نظم"` -> يبني ويعمل عبر الأمر العربي
  ...
  البناء الأول يسجل miss للوحدتين والثاني hit لهما بنفس بصمة المجمّع.
...
وتسلسل `takween-build-events-v1` وحدود العملية ومرحلة المصرّف وأثر التنفيذ عبر
ملف ذي اسم عربي ومسافات،
