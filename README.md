...
- **وضع تكوين:** parser متوافق مع صيغة 0.1 ومع `takween-manifest-v1` متعدد الأهداف؛
  ...
  تقبل خطة البناء جذور `.باء` و`.نظم`، وتبقي `.baa` مدخلا توافقيا، وتختار نَظْم افتراضيا من الأمر العربي
  `نظم` على `PATH`؛ الحقل العربي `المجمع = "غاز"` هو الرجوع الصريح.
- **الاعتمادات:** لا توجد `stdlib` محلية داخل Takween؛ يتم الاعتماد على
  `المكتبة_القياسية.رأسباء` من تثبيت Baa، مع قبول اسم التوافق القديم.
...
[الأهداف.تطبيقي]
النوع = "تنفيذي"
المدخل = "المصدر/الرئيسية.باء"
المصادر = ["المصدر/مساعدة.نظم"]
يعتمد_على = ["هندسة"]

[الأهداف.هندسة]
النوع = "مكتبة"
المدخل = "المصدر/هندسة.باء"
